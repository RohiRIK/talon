...
- [x] M.5 Re-enable macOS + Windows CI matrix (`os: [ubuntu-latest, macos-latest, windows-latest]`) — dropped to stay within free GitHub Actions minutes during development. Restore before v1.0 release to verify cross-platform builds. See `.github/workflows/ci.yml` test + build jobs.
...
> Source: Telegram smoke test revealed tools never executing. Root cause confirmed via
> code review + research of Hermes/OpenClaw patterns.

- [x] 4.20 **Bug 1 — Agent loop stop_reason** `crates/talon-core/src/agent.rs:107` — change `if response.stop_reason == "end_turn" || tool_calls.is_empty()` to `if tool_calls.is_empty()`. GitHub Copilot (OpenAI-compat) returns `finish_reason:"stop"` → `"end_turn"` even when tool_calls are present, causing the loop to exit before running any tool. Drive the loop from tool_calls presence only; `max_iterations` guard remains the infinite-loop backstop. Add a test: loop runs tools when `stop_reason=="end_turn"` + non-empty tool_calls.
- [x] 4.21 **Bug 2 — ApprovalLevel missing from event** `crates/talon-core/src/events.rs` + `approval.rs` — add `approval_level: ApprovalLevel` field to `AgentEvent::ApprovalRequested`; populate it in `ApprovalMembrane::check()`. All gateway match arms use `{ tx, .. }` rest patterns so no gateway edits required. Lets gateways route Safe/NeedsApproval/Dangerous differently.
- [x] 4.22 **Bug 3 — Telegram tool progress messages** `crates/talon-gateway/src/telegram.rs` — on `AgentEvent::ToolCalled { tool_name, args, .. }`, send a brief status message to the chat: `` 🔧 `{tool_name}` · {primary_arg} `` (extract primary arg from args JSON defensively; fall back to tool name). Extract formatting into a pure helper fn for unit testing. Pattern from Hermes: `💻 ls -la...` / `🔍 web_search...`.
- [x] 4.23 **Bug 4 — Telegram inline keyboard approval** `crates/talon-gateway/src/telegram.rs` — replace silent `tx.send(false)` with a proper approval flow: (a) `Safe` → auto-approve immediately; (b) `NeedsApproval`/`Dangerous` → store `tx` in a `tokio::sync::Mutex<HashMap<call_id, Sender<bool>>>`, send inline keyboard `` ⚠️ *Approval Required*\nTool: `{name}` · Risk: {level}\n[✅ Approve] [❌ Deny] ``; (c) `CallbackQuery` handler parses `approve:<call_id>` / `deny:<call_id>`, looks up and removes sender, calls `tx.send(decision)`. `callback_data` ≤ 64 bytes — keep call_id short or hash it. Add unit tests for callback encode/decode and sender lookup round-trip.
- [ ] 4.24 **Live smoke test — Telegram bug fixes** Start `talon` with a real `TELEGRAM_BOT_TOKEN`. Send the following sequence and verify each outcome: (1) plain text message → agent replies with actual content, no "Sure!"; (2) tool-invoking message (e.g. "echo hello") → progress bubble `🔧 \`echo\`` appears before final reply; (3) dangerous tool (e.g. "run rm -rf /tmp/talon_test") → inline keyboard ✅/❌ appears, tap ✅ → tool runs and result returned, tap ❌ → denial message returned; (4) second user DM → bot replies "This is a private assistant." All four checks must pass. Re-register bot token in BotFather if needed.
...
- [x] 5.7 `crates/talon-tools/src/browser/mod.rs` — `BrowserTool` (NeedsApproval) using `headless_chrome` crate (actively maintained CDP client); experimental feature flag `feature = "browser"`
- [x] 5.8 `crates/talon-tools/src/browser/pool.rs` — `BrowserPool`: reuse headless Chrome instances
- [x] 5.9 Tool timeouts: web=30s, browser=60s, mcp=30s (`timeouts.rs`; MCP wired in `adapt_server`)

### Phase 5.1 — Multi-backend web (search + fetch chains) — added 2026-05-29

> Goal: pluggable, config-ordered backend chains so search degrades (Brave → SearXNG → Firecrawl → DDG) and fetch escalates (native Rust → browser → Firecrawl). Native `reqwest`+`dom_smoothie` fetch stays the dependency-free floor — external services are opt-in. Mirrors the provider+registry pattern verified in `NousResearch/hermes-agent` and Open WebUI (see `docs/04_Core_Features/34_Web_Search_Tool.md` §3–§7). All backends return `Pin<Box<dyn Future>>` (no async-trait); errors → `ToolResult::err`.

- [x] 5.10 `crates/talon-tools/src/web/backend.rs` — `SearchBackend` trait (`search(query,count) -> Pin<Box<Future<Output=Result<Vec<SearchResult>, SearchError>>>>`) + `SearchResult{title,url,snippet}` + `SearchError` (thiserror). Refactor `WebSearchTool` to hold an ordered `Vec<Box<dyn SearchBackend>>`, trying each until one returns non-empty; lift existing Brave + DDG logic into `BraveBackend`/`DdgBackend`. Keep tool name/schema/`Safe` unchanged. Tests: per-backend (wiremock), chain fallback (Brave 401 → DDG), all-empty → err.
- [x] 5.11 `crates/talon-tools/src/web/searxng.rs` — `SearxngBackend` (SearchBackend): `GET {base}/search?q=&format=json&pageno=1` → `results[]{title,url,content}`. Constructed from a configured base URL. Tests via wiremock; note public instances often disable `json`.
- [x] 5.12 `crates/talon-tools/src/web/firecrawl.rs` — `FirecrawlBackend`: search (`POST {base}/v1/search`, `Bearer`) as a `SearchBackend`; scrape (`POST {base}/v1/scrape {formats:["markdown"]}`) for fetch. `base` defaults `https://api.firecrawl.dev`, overridable for self-host; key from env/keychain. **Verify exact request/response shapes against Firecrawl docs / Hermes `plugins/web/firecrawl/provider.py` before coding.** Tests via wiremock; no key → backend reports unavailable and is skipped.
- [x] 5.13 `crates/talon-tools/src/web/fetch.rs` — `FetchBackend` trait (`fetch(url) -> Pin<Box<Future<Output=Result<String, FetchError>>>>`). `NativeFetch` (lift `web_extract`'s reqwest+dom_smoothie), `BrowserFetch` (wraps `BrowserPool`, `feature="browser"`), `FirecrawlFetch`. Refactor `WebExtractTool` to an ordered chain with `native` as floor; escalate on empty/err. Tests: native (wiremock), escalation order.
- [x] 5.14 `~/.talon/config.toml [tools.web]` — parse `search_backends` / `fetch_backends` order + per-backend `[tools.web.{brave,searxng,firecrawl}]` (base URLs, key refs). Resolve API keys env → OS keychain. Build the chains from config in `build_gateway_context` (fallback to current defaults when absent). Tests: config parse + chain assembly.
...
### Tasks
- [x] 2.5.1 Deps: **drop** `lancedb` + `arrow-array` (and the `ltm` feature). Add `sqlite-vec`, `tokio-stream`, and `fastembed` (optional, `semantic-search` feature). Register the sqlite-vec extension into the bundled SQLite via `sqlite3_auto_extension` at `Database::open` so every pooled connection has `vec0`.
- [x] 2.5.2 `crates/talon-memory/src/ltm/store.rs` — `MemoryStore` over SQLite: `memories` table `(id, content, category, importance, created_at, accessed_at, decay_score, entities)`, `memories_fts` (FTS5 on content), and `vec_memories` (sqlite-vec `vec0`, `embedding float[384]`). All in `talon.db`; write fact + FTS row + vector in one transaction.
