...
- [ ] 5.10 `crates/talon-tools/src/web/backend.rs` — `SearchBackend` trait (`search(query,count) -> Pin<Box<Future<Output=Result<Vec<SearchResult>, SearchError>>>>`) + `SearchResult{title,url,snippet}` + `SearchError` (thiserror). Refactor `WebSearchTool` to hold an ordered `Vec<Box<dyn SearchBackend>>`, trying each until one returns non-empty; lift existing Brave + DDG logic into `BraveBackend`/`DdgBackend`. Keep tool name/schema/`Safe` unchanged. Tests: per-backend (wiremock), chain fallback (Brave 401 → DDG), all-empty → err.
- [ ] 5.11 `crates/talon-tools/src/web/searxng.rs` — `SearxngBackend` (SearchBackend): `GET {base}/search?q=&format=json&pageno=1` → `results[]{title,url,content}`. Constructed from a configured base URL. Tests via wiremock; note public instances often disable `json`.
