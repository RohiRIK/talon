...
mod tests {
    ...
    async fn save_and_retrieve_recent_message() {
        ...
        store
            .save_message("s1", "user", "hello FTS5")
            .await
            .expect("save");
    ...
    async fn search_returns_matching_message() {
        let store = make_store().await;
        store
            .save_message("s2", "user", "Rust is fast")
            .await
            .expect("save");
        store
            .save_message("s2", "assistant", "hello world")
            .await
            .expect("save");
    ...
    async fn search_returns_empty_on_no_match() {
        let store = make_store().await;
        store
            .save_message("s3", "user", "something else")
            .await
            .expect("save");
    ...
    async fn recent_messages_oldest_first_ordering() {
        ...
        store
            .save_message("s4", "assistant", "second")
            .await
            .expect("2");
    ...
    async fn arc_dyn_store_dispatches_correctly() {
        ...
        store
            .save_message("s5", "user", "dyn dispatch")
            .await
            .expect("save");
